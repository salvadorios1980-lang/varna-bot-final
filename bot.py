...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(PRICE_TEXT + "\n\n👇 Какво искаш да пуснеш? Избери категория:", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))
    return CATEGORY
...
async def ad_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Супер, получих обявата! 👇\n\nЗа колко време да я закачим?", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))
    return DURATION
...
async def duration_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(f"✅ Обявата ти е пусната в канала!\n\nИзбра: {duration}\n\n{PRICE_TEXT}", parse_mode="Markdown")
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[CommandHandler("start", start), CommandHandler("newpost", start)],
        states={
        ...
        fallbacks=[CommandHandler("cancel", cancel), CommandHandler("price", price)]
    )
    app.add_handler(conv)
    app.add_handler(CommandHandler("price", price))
    app.add_handler(CommandHandler("cenorazpis", price))
    print("Бота тръгна...")
    app.run_polling()
